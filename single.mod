...
param z{i in all_numcons, j in all_cons[i]} = sum{k in all_events[j]} x[all_primitives[k,1], all_primitives[k,2], all_primitives[k,3]] *
...
zstar[i,j] = sum{k in agent_events[j]} xstar[agent_primitives[k,2],agent_primitives[k,3]] * 
